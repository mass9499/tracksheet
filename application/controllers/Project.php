...
class Project extends CI_Controller {
	...
	public function index()
	{
		...
		$data['subtitle'] = 'Institutions List';
	...
	{
		$data['title'] = 'Institutions';
		$data['subtitle'] = 'View Institutions';
		$data['project_data'] = $this->project_model->get_project_details($project_id);
		$data['project_tasks'] = $this->db->order_by('project_task_id', 'ASC')->get_where('project_tasks_tbl', array('project'=>$data['project_data']['project_id']))->result_array();
		$assigned_to = explode(";", $data['project_data']['assigned_to']);
		
		$data['assigned_to'] = $this->project_model->get_assigned_users($assigned_to);
		
		$project_uniq_id = $data['project_data']['unique_id'];
		$data['get_design_pages'] = $this->project_model->get_project_designs($project_uniq_id);
		$this->load->view('admin/templates/header', $data);
		$this->load->view('admin/project_view');
		$this->load->view('admin/templates/footer');
	}
	...
	{
		$data['title'] = 'Institutions';
		$data['subtitle'] = 'Edit Institutions';
		...
		$data['client_projects'] =  $this->client_model->get_client_projects();
	...
	{
		$data['title'] = 'Institutions';
		$data['subtitle'] = 'Create Institutions';
	...
	public function create_project_save()
	{
        ...
        $data['client_pro_id'] = $this->input->post('client_pro_id');
        $data['total_timeline'] = $this->input->post('total_timeline');

     // print_r( $data['project_type']);die;
